class Socket
{
	...
	Connect(Address)
	{
		...
		this.onErrorConn(this.GetLastError())
	}
	...
	Bind(Address)
	{
		...
		this.onErrorBind(this.GetLastError())
	}
	...
	Disconnect()
	{
		...
		if (DllCall("Ws2_32\closesocket", "UInt", this.Socket, "Int") == -1)
			this.onErrorDisconn(this.GetLastError())
		this.Socket := -1
		this.onDisConnected()
		return 1
	}
	...
	Send(pBuffer, BufSize, Flags:=0)
	{
		if ((r := DllCall("Ws2_32\send", "UInt", this.Socket, "Ptr", pBuffer, "Int", BufSize, "Int", Flags)) == -1)
			this.onErrorSend(this.GetLastError())
		else
		  this.onSent()
		return r
	}
	...
	OnMessage(wParam, lParam, Msg, hWnd)
	{
		...
		if (lParam & this.FD_READ)
			...
			this.EventProcUnregister(), this.OnDisconnected()
	}
